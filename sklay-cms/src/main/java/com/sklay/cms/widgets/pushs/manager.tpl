...
<div class="row-fluid">
	<div class="span3">
		<div class="nav">
			<a id="saveTypeBtn" class="btn" data-toggle='modal' data-target='#saveType' href="${action}/pushs/saveTypeView">新增</a>
			<a id="editTypeBtn" class="btn" disabled='disabled' data-toggle='modal' data-target='#editType' href="${action}/pushs/saveTypeView?id=${newsType!!}">修改</a>
			<a id="removeTypeBtn" class="btn" disabled='disabled' href="${action}/pushs/removeType?id=${newsType!!}">删除</a>
		</div>
	...
	<div class="span9">
		<div class="row-fluid">
			...
			<div class="pull-right">
				...
				<#if page.content?size gt 0>
					<button autocomplete="off" class="btn" disabled='disabled' id="widget${widget.id}-edit" rel="${action}/pushs/getNews">修改</button>
					<button autocomplete="off" class="btn" disabled='disabled' id="widget${widget.id}-remove" rel="${action}/pushs/deleteNews">删除</button>
				</#if>
			</div>
		</div>
		...
		<#if leafGroups??>
			<form action="${action}/pushs/addNews" method="post" id="newsManagerForm">
				<fieldset>
					<legend>编辑内容</legend>
					<input type="hidden" name="id" value="">
					<input type="hidden" name="biz" value="pushs">
					<label>标题</label>
...
<script>
	...
	newsParam.deleteNews = '${action}/pushs/deleteNews';
	newsParam.removeType = '${action}/pushs/removeType';
	newsParam.topNews = '${action}/pushs/topNews';
	newsParam.editNews = '${action}/pushs/editNews';
	newsParam.allGroups = ${allGroups?default('[]')};
	newsParam.loadChildTypeUrl = '${action}/pushs/loadChildren';
	newsParam.reloadUrl = '${view}?newsType=';
	newsParam.changeTypeUrl = '${action}/pushs/changeType';
</script>
